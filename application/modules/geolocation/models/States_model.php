...
class States_model extends CI_model
{
    ...
    protected $error_prefix = 'STA';
    protected $tb1 = 'geo_states';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->states_model->geolocation(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function states($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, 'name = ' . $this->db->escape($input_post['name']) . " AND id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->states($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->states($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                ...
                'iso_cods' => strtoupper(@$datas['iso_cods']),
                'code3' => strtoupper(@$datas['code3']),
                'updated_by' => get_user()['id']
            ];
            $exist = $this->states_model->detailed(null, 'row_status = 1 AND name = ' . $this->db->escape($data_post['name']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->states($id, $data_post, 'PATCH');
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->states(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
