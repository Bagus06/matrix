...
class Countries_model extends CI_model
{
    ...
    protected $error_prefix = 'CTR';
    protected $tb1 = 'geo_countries';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->countries_model->geolocation(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function countries($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, '(name = ' . $this->db->escape($input_post['name']) . ' OR phone_code = ' . $this->db->escape($input_post['phone_code']) . ' OR iso2 = ' . $this->db->escape($input_post['iso2']) . ' OR iso3 = ' . $this->db->escape($input_post['iso3']) . ") AND id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->countries($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->countries($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                ...
                'iso2' => strtoupper(@$datas['iso2']),
                'iso3' => strtoupper(@$datas['iso3']),
                'phone_code' => @$datas['phone_code'],
                'updated_by' => get_user()['id']
            ];
            $exist = $this->countries_model->detailed(null, 'row_status = 1 AND name = ' . $this->db->escape($data_post['name']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->countries($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['name'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->countries(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
