...
{
    protected $module_name = 'geolocation';
    protected $error_prefix = 'GEO';
    protected $tb1 = 'geo_villages';
    protected $tb2 = 'geo_districts';
    protected $tb3 = 'geo_cities';
    protected $tb4 = 'geo_states';
    protected $tb5 = 'geo_countries';
    ...
    public function detailed($item_id = 0, $whereclause = '')
    {
        ...
        if (!empty($whereclause) || !empty($item_id)) {
            $query = "SELECT geo_villages.id, geo_countries.name, geo_countries.id, geo_states.name, geo_states.id, geo_cities.name, geo_cities.id, geo_districts.name, geo_districts.id, geo_villages.name FROM $this->tb1";
            $query .= " INNER JOIN $this->tb2 ON $this->tb1.district_id = $this->tb2.id";
            $query .= " INNER JOIN $this->tb3 ON $this->tb2.city_id = $this->tb3.id";
            $query .= " INNER JOIN $this->tb4 ON $this->tb3.state_id = $this->tb4.id";
            $query .= " INNER JOIN $this->tb5 ON $this->tb4.country_id = $this->tb5.id";
            $query .= ' WHERE ';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->geolocation_model->geolocation(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function geolocation($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
                ...
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->geolocation($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->geolocation($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            ...
            $exist = $this->geolocation_model->detailed(null, 'row_status = 1 AND test = ' . $this->db->escape($data_post['test']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->geolocation($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['test'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->geolocation(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
