...
class Cities_model extends CI_model
{
    ...
    protected $error_prefix = 'CTY';
    protected $tb1 = 'geo_cities';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->cities_model->geolocation(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function cities($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
                ...
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->cities($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->cities($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                ...
                'city_code' => strtoupper(@$datas['city_code']),
                'updated_by' => get_user()['id']
            ];
            $exist = $this->cities_model->detailed(null, 'row_status = 1 AND name = ' . $this->db->escape($data_post['name']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->cities($id, $data_post, 'PATCH');
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->cities(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
