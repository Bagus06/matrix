...
class University_courses_model extends CI_model
{
    protected $module_name = 'university_courses';
    protected $error_prefix = 'UCSE';
    protected $tb1 = 'university_courses';
    ...
    public function detailed($item_id = 0, $whereclause = '')
    {
        ...
        if (!empty($whereclause) || !empty($item_id)) {
            $query = "SELECT $this->tb1.*, $this->tb2.university_name, $this->tb2.short_name FROM $this->tb1";
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->courses(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function courses($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, '(course_name = ' . $this->db->escape($detailed['data']['course_name']) . ' AND university_id = ' . $this->db->escape($detailed['data']['university_id']) . ") AND $this->tb1.id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->courses($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->courses($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            ...
            $data_post = [
                ...
                'course_code' => strtoupper(substr($datas['course_code'], 0, 20)),
            ...
            // Exploding date period and format date for insert to DB
            if (!empty($datas['discount_date_periode'])) {
                $discount_date_periode = explode(' - ', @$datas['discount_date_periode']);
                if (!empty($discount_date_periode[0]) && !empty($discount_date_periode[1])) {
                    $date_start = DateTime::createFromFormat('d/m/Y', $discount_date_periode[0]);
                    $date_end = DateTime::createFromFormat('d/m/Y', $discount_date_periode[1]);

                    $data_post['discount_duration_start'] = $date_start->format('Y-m-d');
                    $data_post['discount_duration_end'] = $date_end->format('Y-m-d');
                }
            } else {
                unset($data_post['discount_duration_start']);
                unset($data_post['discount_duration_end']);
            }

            $exist = $this->detailed(null, "$this->tb1.row_status = 1 AND (course_name = " . $this->db->escape($data_post['course_name']) . ' AND university_id = ' . $this->db->escape($data_post['university_id']) . ')');
            ...
            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['course_name'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->courses(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
