...
class Example_model extends CI_model
{
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->test(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
                ...
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->test($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->test($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            ...
            $exist = $this->detailed(null, 'row_status = 1 AND test = ' . $this->db->escape($data_post['test']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->test($id, $data_post, 'PATCH');
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->test(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
