...
class Apps_permission_group_relations_model extends CI_model
{
    protected $module_name = 'apps_permission_groups';
    protected $error_prefix = 'APGR';
    protected $tb1 = 'apps_permission_group_relations';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->group_relations(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function group_relations($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, 'group_code = ' . $this->db->escape($input_post['group_code']) . ' AND feature_code = ' . $this->db->escape($input_post['feature_code']) . " AND id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->group_relations($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->group_relations($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                'group_code' => substr(strtoupper(trim($datas['group_code'])), 0, 15),
                'feature_code' => substr(strtoupper(trim($datas['feature_code'])), 0, 15),
                'updated_by' => get_user()['id']
            ];

            $exist = $this->detailed(null, 'row_status = 1 AND group_code = ' . $this->db->escape($data_post['group_code']) . ' AND feature_code = ' . $this->db->escape($data_post['feature_code']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->group_relations($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['group_code'] . " -> " . $data_post['feature_code'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->group_relations(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
    ...
    public function feature_accessed($user_id = null)
    {
        $output = [
            'status' => FALSE,
            'code' => null,
        ...
        $user_detailed = $this->users_model->detailed($user_id);
        if (!empty($user_detailed['data'])) {

            $escaped = array_map([$this->db, 'escape'], json_decode(@$user_detailed['data']['permission_group'], true));
            if (!empty($escaped)) {
                $permission_group  = '(' . implode(', ', $escaped) . ')';
            }
            ...
            $get_params = [
                ...
                'bypass' => false,
                'whereclause' => ((!empty(json_decode(@$user_detailed['data']['permission_group'], true))) ? 'group_code IN ' . @$permission_group : 'group_code = 0')
            ];
            $features = $this->group_relations(0, $get_params, 'GET');
