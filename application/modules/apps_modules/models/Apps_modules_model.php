...
class Apps_modules_model extends CI_model
{
    protected $module_name = 'apps_modules';
    protected $error_prefix = 'MODL';
    protected $tb1 = 'apps_modules';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->apps_modules(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function apps_modules($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, 'module_code = ' . $this->db->escape($input_post['item']) . " AND id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->apps_modules($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->apps_modules($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                'module_code' => preg_replace('/[^A-Za-z_]/', '', 'MD_' . strtoupper($datas['module_code'])),
                'module_title' => $datas['module_title'],
                'description' => $datas['description'],
                'status' => ((@$datas['status'] === 'on') ? 'ACTIVE' : 'INACTIVE'),
                'sys_lock' => ((@$datas['sys_lock'] === 'on') ? true : false),
                'updated_by' => get_user()['id']
            ];
            $exist = $this->detailed(null, 'row_status = 1 AND module_code = ' . $this->db->escape($data_post['module_code']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->apps_modules($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['module_code'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    if (empty($data_post['description'])) {
                        unset($data_post['description']);
                    }

                    $output = $this->apps_modules(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
