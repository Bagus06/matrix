...
class Apps_module_features_model extends CI_model
{
    protected $module_name = 'apps_module_features';
    protected $error_prefix = 'FEAT';
    protected $tb1 = 'apps_module_features';
    protected $tb2 = 'apps_modules';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->features(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function features($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
        ...
        if (strtoupper($type) === 'POST') {
            ...
            */
            ...
            $query .= " INNER JOIN $this->tb2 ON $this->tb1.module_id = $this->tb2.id";
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                ...
                'feature_code' => preg_replace('/[^A-Za-z_]/', '', strtoupper(('FT_' . $datas['module_code'] . '_' . $datas['feature_code']))),
                'feature_title' => $datas['feature_title'],
                'description' => $datas['description'],
                'status' => ((@$datas['status'] === 'on') ? 'ACTIVE' : 'INACTIVE'),
                'sys_lock' => (($datas['sys_lock'] === 'on') ? 1 : 0),
                'updated_by' => get_user()['id']
            ];
            $exist = $this->detailed(null, "$this->tb1.row_status = 1 AND feature_code = " . $this->db->escape($data_post['feature_code']));

            if (!empty($id)) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->features($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['feature_code'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->features(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
