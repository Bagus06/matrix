...
class Leads_model extends CI_model
{
    ...
    private function whereclause_system($params = [])
    {
        ...
        if (!$params['bypass']) {
            ...
            if (user_group_check('GR_MKTST', get_user()['id']) && (strtoupper(get_user()['username']) !== 'DEVELOPER') && (strtoupper(@$params['real_page']) === 'FOLLOWUP')) {
                $output .= " AND leads.status = 'PENDING' AND follow_up_date < '" . date('Y-m-d') . "'";
            }
        }
    ...
    public function leads($item_id = 0, $datas = '', $type = '')
    {
        ...
        if (strtoupper($type) === 'POST') {
            ...
            */
            ...
            $query .= $this->whereclause_system($datas);
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                ...
                'whatsapp_number' => substr($datas['whatsapp_number'], 0, 15),
