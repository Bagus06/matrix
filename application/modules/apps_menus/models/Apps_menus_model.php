...
class Apps_menus_model extends CI_model
{
    protected $module_name = 'apps_menus';
    protected $error_prefix = 'APMS';
    protected $tb1 = 'apps_menus';
            ...
            if (is_array(@$data["search"])) {
                ...
                if ($wc_query['status']) {
                    if (empty($wc_query['data'])) {
                        ...
                        $query_builder = $this->menus(0, $get_params, "GET");
                    }
                } else {
                    $output = $wc_query;
                    return $output;
    ...
    public function menus($item_id = 0, $datas = '', $type = '')
    {
        $output = [
            'status' => FALSE,
            'code' => null,
    ...
    public function restore($id)
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($detailed['data'])) {
                $exist = $this->detailed(null, 'test = ' . $this->db->escape($input_post['item']) . " AND id != $id");

                $output = $exist;
                if ($exist['status']) {
                    if (!empty($exist['data'])) {
                        if (@$input_post['replace']) {
                            $delete = $this->menus($exist['data']['id'], 0, 'DELETE');

                            $output = $delete;
                            if ($delete['status']) {
                                $output = $this->menus($id, ['row_status' => 1, 'updated_by' => get_user()['id']], 'PATCH');
                            }
                        } else {
                            $output = [
                                'status' => TRUE,
    ...
    public function create_and_edit($id = null, $datas = null)
    {
        ...
        if (!empty($datas)) {
            $data_post = [
                'feature_code' => @(($datas['feature_code'] === 'null') ? NULL : $datas['feature_code']),
                'icon' => (($datas['icon'] === 'null') ? NULL : $datas['icon']),
                'display_title' => $datas['display_title'],
                'description' => (($datas['description'] === 'null') ? NULL : $datas['description']),
                'url' => (($datas['url'] === 'null') ? NULL : $datas['url']),
                'menu_status' => (($datas['menu_status'] === 'true') ? TRUE : FALSE),
                'order_menu' => $datas['order_menu'],
                'parent_menu_id' => (($datas['parent_menu_id'] === 'null') ? NULL : $datas['parent_menu_id']),
                'visible' => (($datas['visible'] === 'true') ? TRUE : FALSE),
                'sys_lock' => (($datas['sys_lock'] === 'true') ? TRUE : FALSE),
                'updated_by' => get_user()['id']
            ];
            $exist = $this->detailed(null, 'row_status = 1 AND (display_title = ' . $this->db->escape($data_post['display_title']) . ' AND (parent_menu_id = ' . $this->db->escape($data_post['parent_menu_id']) . ' OR parent_menu_id IS NULL))');
            $detailed = $this->detailed($id);

            if (!empty($detailed['data'])) {
                if (($id == @$exist['data']['id']) || empty($exist['data'])) {
                    $output = $this->menus($id, $data_post, 'PATCH');
                    ...
                    $output = [
                        ...
                        'replace_code_value' => [
                            'message' => "The data '" . $data_post['display_title'] . "' already exists, please check the data again or enter other data"
                        ],
                        'redirectUrl' => null,
                        'debug' => null,
                        'data' => null
                ...
                if (empty($exist['data'])) {
                    ...
                    $output = $this->menus(null, $data_post, 'POST');
                } else {
                    $output = [
                        'status' => TRUE,
                        'code' => "$this->error_prefix-CNU-E001",
    ...
    public function build_menu_tree(array $data)
    {
        $items = [];
        $output  = [];
        ...
        foreach ($data as $row) {
            $row['id'] = $row['id'];
            $row['parent_menu_id'] = $row['parent_menu_id'];
            $row['type'] = 'child';
            $row['menu_status'] = (bool)$row['menu_status'];
            $row['visible']     = (bool)$row['visible'];
            $row['sys_loc']     = (bool)$row['sys_lock'];
            $row['children']    = [];
            $items[$row['id']] = $row;
        }
    ...
    public function menu_tree()
    {
        ...
        $data_menu = $this->menus(0, $get_params, 'GET');

        if ($data_menu['status'] && !empty($data_menu['data']['data'])) {
            $menu_tree_array = $this->build_menu_tree($data_menu['data']['data']);

            if (!empty($menu_tree_array)) {
                $output = [
                    'status' => TRUE,
                    'code' => "",
                    'replace_code_value' => null,
